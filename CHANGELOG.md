...
- **`--save`** — stages the top drift findings (default 10, `--save-top` up to 24) plus one run summary as **proposals** in your ClariLayer Context Inbox, in a single `propose_batch` call to the hosted MCP endpoint. You review each proposal; accepted items land as `asserted` schema notes — never anything stronger. Supporting flags: `--key` / `CLARILAYER_CONTEXT_KEY`, and `--dry-run` to print the exact would-be request body with zero network (no key needed).
