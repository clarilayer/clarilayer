...
This is the **front door** to ClariLayer — the docs, examples, recipes, and (soon) a thin setup CLI that help you connect your AI agent to the hosted ClariLayer MCP server.
...
ClariLayer is a trust product, so the docs hold a hard line: today the only context statuses are **`asserted`** and **`caveat`**. A stronger **`verified`** status is *not shipped* — it's a documented fast-follow. Please don't add copy that describes context as "verified." If a change implies it, we'll ask you to reword it.
