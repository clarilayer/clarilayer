...
It reports phantom columns (documented but missing from the warehouse), models missing from the catalog, type family mismatches, and hollow descriptions. Add `--save` to stage the top findings as proposals in your Context Inbox for review — see [CLI.md](./CLI.md#npx-clarilayer-dbt-check) and the [dbt-check guide](https://clarilayer.com/docs/guides/dbt-check).
