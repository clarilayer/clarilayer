 ...
 * agent reaches for ClariLayer proactively. Mirrors the product's
 * RECOMMENDED_AGENT_STANZA (apps/web/lib/mcp/install-commands.ts), synced
 * 2026-07-30 at capability v51 — this copy MUST be re-synced whenever the
 * product stanza revs (the server's `get_project_stanza` verb returns the
 * canonical text). Truthful by construction: asserted/caveat only, never
 * "verified".
 */
