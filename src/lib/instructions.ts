...
4. This CLI request supplies no authenticated Connect project report scope. Use the generic managed_block, without instance metadata. Do not invent project or installation identifiers, remove or rebind a scoped current block, or call sync_instruction_setup. If an existing block contains scoped instance metadata, leave it unchanged and report conflict/manual review, even when its version, hash and workflow payload match the current server values. Matching workflow bytes alone do not establish that a scoped installation is current. Use Connect your AI -> Update my AI setup for that scoped update or reporting step.
5. Before writing, retain the exact surrounding bytes. After writing, re-read the complete target and check that the installed range equals the returned managed_block, its version and hash agree, and all surrounding bytes are unchanged. Only then report installed. Report already current only when the entire existing managed range is byte-identical to the generic managed_block, with no instance metadata, and the target's preamble requirements pass. Report conflict/manual review for every other conflict. Report connection, local installation and observed context use separately. A printed request is not an installed file or evidence of recall.
